...
class AddmmOutTest : public ::testing::Test {
 protected:
  ...
  void run_and_verify(
    ...
    SlimTensor slim_input = torch::standalone::create_tensor_from_blob(
        at_input.data_ptr(),
        at_input.sizes(),
        at_input.strides(),
        at_input.scalar_type(),
        cuda_device_);
    ...
    // Create output tensors for both implementations. The 'out' tensor for
    // addmm_out must have the same shape as the final result.
    at::Tensor at_out =
        at::empty({at_mat1.size(0), at_mat2.size(1)}, at_input.options());
    ...
    std::vector<int64_t> contiguous_strides =
        torch::standalone::compute_contiguous_strides(expected_shape);
    SlimTensor slim_out = torch::standalone::create_empty_tensor(
        expected_shape, contiguous_strides, at_out.scalar_type(), cuda_device_);
...
TEST_F(AddmmOutTest, BetaIsZero) {
  ...
  at::Tensor at_input =
      at::full({8, 8}, std::numeric_limits<float>::quiet_NaN(), options);
