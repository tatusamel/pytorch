...
#include <torch/csrc/inductor/aoti_standalone/cpu/transpose.h>
#include <torch/csrc/inductor/aoti_standalone/cuda/transpose.h>

using torch::standalone;
...
TEST(Transpose, TransposeBasic) {
  ...
  SlimTensor slim_tensor_self = torch::standalone::create_tensor_from_blob(at_tensor.data_ptr(), c10::IntArrayRef(at_tensor.sizes().data, at_tensor.dim()), c10::IntArrayRef(at_tensor.strides().data(), at_tensor.dim()), at::kFloat);

  AtenTensorHandle result_handle = nullptr;
  AOTITorchError err = aoti_torch_cpu_transpose_int(reinterpret_cast<AtenTensorHandle>(&slim_tensor_self), 0, 1, &result_handle);
