    ...
    HEADDIM_SWITCH(params.d, [&] {
      BOOL_SWITCH(params.is_causal, Is_causal, [&] {
        if (params.num_splits <= 1 &&
          ...
          FLASH_NAMESPACE::run_mha_fwd_splitkv_dispatch<elem_type, kHeadDim, Is_causal>(
              params, stream);
        }
      });
    });
...
std::tuple<SlimTensor, SlimTensor> set_params_splitkv(
  ...
  SlimTensor softmax_lse_accum = torch::standalone::create_empty_tensor({}, {}, c10::kFloat, c10::kCUDA);
  SlimTensor out_accum = torch::standalone::create_empty_tensor({}, {}, c10::kFloat, c10::kCUDA);
...
mha_fwd(
  ...
  SlimTensor out = torch::standalone::create_empty_tensor({}, {}, q.dtype(), q.device());
  if (out_.has_value()) {
    out = out_.value();
    TORCH_CHECK(
  ...
  SlimTensor p = torch::standalone::create_empty_tensor({}, {}, q.dtype(), q.device());
  ...
  auto rng_state = empty(
      {2}, c10::TensorOptions().dtype(c10::kUInt64).device(c10::kCUDA));
  auto _unused = empty(
      {}, c10::TensorOptions().dtype(c10::kUInt64).device(c10::kCUDA));
  if (p_dropout > 0.0) {
    TORCH_CHECK(false, "Standalone Flash Attention does not support dropout yet.");
  }
...
mha_varlen_fwd(
  ...
  SlimTensor block_table = torch::standalone::create_empty_tensor({}, {}, c10::kInt, c10::kCUDA);
  ...
  if (seqused_k.has_value()) {
    ...
    TORCH_CHECK(seqused_k_.device().is_cuda(), "seqused_k must be on CUDA device");
  ...
  auto softmax_lse =
      empty({num_heads, total_q}, opts.dtype(c10::kFloat));
  SlimTensor p = torch::standalone::create_empty_tensor({}, {}, q.dtype(), q.device());
  // Only return softmax if there's dropout to reduce compilation time
  if (return_softmax) {
    TORCH_CHECK(
  ...
  // Keep references to these tensors to extend their lifetime
  SlimTensor softmax_lse_accum = torch::standalone::create_empty_tensor({}, {}, q.dtype(), q.device());
  SlimTensor out_accum = torch::standalone::create_empty_tensor({}, {}, q.dtype(), q.device());
  if (seqlenq_ngroups_swapped) {
  ...
  // and "offset" → "_unused"
  auto rng_state =
      empty({2}, c10::dtype(c10::kUInt64).device(c10::kCUDA));
...
std::tuple<SlimTensor, SlimTensor> mha_fwd_kvcache(
  ...
  SlimTensor q_padded = torch::standalone::create_empty_tensor({}, {}, q.dtype(), q.device());
  SlimTensor kcache_padded = torch::standalone::create_empty_tensor({}, {}, kcache.dtype(), kcache.device());
  SlimTensor vcache_padded = torch::standalone::create_empty_tensor({}, {}, vcache.dtype(), vcache.device());
  if (head_size_og % 8 != 0) {
  ...
  SlimTensor k = torch::standalone::create_empty_tensor({}, {}, q.dtype(), q.device());
  SlimTensor v = torch::standalone::create_empty_tensor({}, {}, q.dtype(), q.device());
  SlimTensor k_padded = torch::standalone::create_empty_tensor({}, {}, q.dtype(), q.device());
  SlimTensor v_padded = torch::standalone::create_empty_tensor({}, {}, q.dtype(), q.device());
  if (k_.has_value()) {
    TORCH_CHECK(
  ...
  if (head_size_og % 8 != 0) {
    ...
    if (k_.has_value()) {
      ...
      const_cast<SlimTensor&>(kcache).copy_(kcache_padded.narrow(-1, 0, head_size_og));
      const_cast<SlimTensor&>(vcache).copy_(vcache_padded.narrow(-1, 0, head_size_og));
